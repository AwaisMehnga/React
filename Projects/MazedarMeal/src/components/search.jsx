...
export default function Search(props){
    ...
    const getRecipe = async () => {
        try {
          const response = await Axios.get(`https://www.themealdb.com/api/json/v1/1/search.php?s=${props.recipeName}`);
    ...
    <div className='search'>
        <input list='sug' className='recipeInput' type="text" placeholder='Enter Recipe...' onChange={(e)=>{
            props.setRecipeName(e.target.value); 
            e.target.value.length==1 &&setChar(e.target.value);
        
        }} />
        <datalist id='sug'>
            {recipeNames && recipeNames.meals && recipeNames.meals.map((recipe, index) => (
                <option key={index}value={recipe.strMeal}></option>
            ))}
        </datalist>
        <button className='recipeBTN' onClick={()=>getRecipe(props.recipeName)}>Recipe</button>
